...
describe('settingsStore migration', () => {
  ...
  it('v1 state missing newer fields survives migration (zustand merge fills defaults)', () => {
    // Simulates a v1 state saved before autoSyncOnConnect, keepAwake, etc. were added
    const oldV1State = {
      setupComplete: true,
    ...
    // Migration should not touch v1 state — newer fields like autoSyncOnConnect,
    // keepAwakeEnabled, keepAwakeMinutes, keepAwakeAlways are NOT added by migration.
    // Zustand's merge strategy fills them from the store's initial state at runtime.
