...
import { useAncestorStore } from '../../../src/renderer/stores/ancestorStore'
...
describe('ancestorStore persistence round-trip (desktop)', () => {
  beforeEach(async () => {
    resetIpcStorageFake()
  ...
  it('commit writes the envelope to ipcStorage', async () => {
    ...
    const raw = ipcStorageStore['jotbunker-ancestor']
  ...
  it('tombstones survive a commit -> persist -> rehydrate cycle', async () => {
    ...
    // Clear in-memory; setState's persist write would clobber the fixture.
    useAncestorStore.setState({ record: null })
    await flush()
    ipcStorageStore['jotbunker-ancestor'] = raw!
