...
Locked lists sync bidirectionally using the same automatic three-way merge as regular lists. Both sides' edits survive without picking a winner. The biometric gate is local to the phone; syncing doesn't require re-authentication. When the computer receives locked list data from the phone, it merges and displays it directly.
