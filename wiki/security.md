...
- **Tier 3 (replace the app rule with a port rule).** Delete the installer's `Jotbunker` rule. Create a new inbound rule for TCP port 8080 only, **scoped to your phone's specific LAN IP** (not `Any`), Private profile only.  **You should not leave Remote IP as `Any`** — that opens port 8080 to every host on every network the rule applies to, including public Wi-Fi, and undoes the whole point of tightening. The default rule's LocalSubnet protection is gone once you remove it; a naive port rule is how people accidentally expose themselves while trying to be more strict. Tightest setup when done right; also means you'll need to update the rule if you change the sync port in Settings.
